#include "pid-auton/auton_functions.h"
#include "pid-auton/pid_control.h"
#include "main.h"
#include "iostream"
#include <bits/stdc++.h>
...
namespace auton {
    ...
    void driveAndTurn(double tiles, double angle, double linearMaxVelocity, double turnMaxVelocity, double timeoutMs, double MOE){
        ...
        std::vector<double> leftMotorPositions = left_mg.get_positions();
        double initLeftMoterDegree = std::accumulate(leftMotorPositions.begin(), leftMotorPositions.end(), 0) / leftMotorPositions.size();

        std::vector<double> rightMotorPositions = right_mg.get_positions();
        double initRightMoterDegree = std::accumulate(rightMotorPositions.begin(), rightMotorPositions.end(), 0) / rightMotorPositions.size();
